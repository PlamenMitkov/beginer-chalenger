...
impl Library {
        ...
        for (book, status) in &mut self.books {
            if book.isbn == isbn {
                match status {
                    ...
                    BookStatus::Borrowed => return Err("Book is already borrowed"),
                }
            }
        }
        ...
        for (book, status) in &mut self.books {
            if book.isbn == isbn {
                match status {
                    ...
                    BookStatus::Available => return Err("Book wasn't borrowed"),
                }
            }
        }
        Err("Book not found in library")
    ...
    fn list_books(&self) {
        println!("Library Book Inventory:");
        for (book, status) in &self.books {
            ...
            println!(
                "\"{}\" by {} ({}) - {}",
                book.title, book.author, book.year, status_str
            );
        }
        println!();
    }
